...
class grocery_CRUD_Field_Types
{
	...
	protected function get_field_input($field_info, $value = null)
	{
		...
		$field_info->input = $field_info->input; //.'<h1>$shown_field_info</h1>';
		return $field_info;
	}
		...
		if(isset($state_info->search) && $state_info->search !== '')
		{
			if (!empty($this->relation)) {
				...
				foreach ($this->relation as $relation_name => $relation_values) {
					$ruki_1 = $relation_values[2];
					$ruki_rel[$this->_unique_field_name($relation_name)] = $this->_get_field_names_to_search_ruki($relation_values);

					$temp_relation[$this->_unique_field_name($relation_name)] = $this->_get_field_names_to_search($relation_values);
        }
      }
// echo '<pre>';print_r($ruki_rel);die();
			
			if (is_array($state_info->search)) { //search multiple field
					foreach ($state_info->search as $search_field => $search_text) {
							if (isset($temp_relation[$search_field])) {
									if (is_array($temp_relation[$search_field])) {
											$tmp2 = $ruki_rel[$search_field]. ' LIKE \'%' . $this->basic_model->escape_str($search_text) . '%\' ';
											...
											// if (!empty($temp_where_query_array)) {
													// $this->where('(' . implode(' OR ', $temp_where_query_array) . ')', null); //ASLI
													$this->where($tmp2, null);
											// }
								...
								if(isset($this->search_mask[$search_field])){
									$this->like( $this->search_mask[$search_field] , $search_text);
								}else{
									$this->like($this->get_table().'.'.$search_field, $search_text);
								}
							}
					}
// echo '<pre>';print_r($this);die();
			} elseif ($state_info->search->field !== null) { //search 1 field only
	...
	protected function _get_field_names_to_search_ruki(array $relation_values) //enable search with set_relation('a','b','{a}/{a+1} {b}')
	{
//$relation_values = (
//             [0] => term_masuk
//             [1] => timing
//             [2] => {year_start}/{year_start+1} {semester}
//             [3] => 
//             [4] => 
//         )
		if(!strstr($relation_values[2],'{')) {
			...
			preg_match_all("/( } ( (?: [^{}]* | (?1) )* ) { )/x",$relation_values[2],$hsl);
			$strg = array();
			foreach($hsl[2] as $tx){
				$strg[] = "'$tx'";
			}
			$gabung = array();
			$cnt = 0;
			foreach($field_names_array as $rel_fld){
				$gabung[] = $rel_fld;
				if(isset($strg[$cnt])){
					$gabung[] = $strg[$cnt];
				}
			...
			return ' concat( '.implode(',',$gabung).' ) ';
		}
// return (
//             [0] => j529d3053.year_start
//             [1] => j529d3053.year_start+1
//             [2] => j529d3053.semester
//         )
	}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->columnNoSearch	= $this->columnNoSearch;
		$data->columnNoSorting	= $this->columnNoSorting;
		...
		if(!$ajax)
		{
			...
			$this->_theme_view('list_template.php',$data); //columns
		}
		else
		{
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	function getEditUrl($primary_key = null)
	{
		if($primary_key === null)
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $search_mask				= null;
	protected $columnNoSearch				= null;
	protected $columnNoSorting				= null;
	...
	protected $shown_field_info 	= null;
	...
	///by ruki: search with specific string format (to be used with callback_column and callback_read_field)
	// e.g.:
	//	$c->search_mask('year_start',"concat(timing.year_start,'/',timing.year_start+1)");
	//	$c->callback_column('year_start',function($v,$pk){return strval($v).'/'.strval($v+1).' '.$this->Coursesmodel->isCurTerm($pk->id);});
	public function search_mask(){
		$args = func_get_args();
		if(isset($args[0]) && is_array($args[0])){
			$args = $args[0];
		}
		$this->search_mask[$args[0]] = $args[1];
		return $this;
	}
	///by ruki: hide search in certain column
	public function columnNoSearch(){
		$args = func_get_args();
		if(isset($args[0]) && is_array($args[0])){
			$args = $args[0];
		}
		$this->columnNoSearch = $args;
		return $this;
	}
	///by ruki: disable sorting in certain column
	public function columnNoSorting(){
		$args = func_get_args();
		if(isset($args[0]) && is_array($args[0])){
			$args = $args[0];
		}
		$this->columnNoSorting = $args;
		return $this;
	}
	///by ruki: execute where() on ajax search
	public function ajaxWhere($w){
	    $this->pre_render();
                if($this->theme === null)
				...
				$state_info = $this->getStateInfo();
				$state_info->search = null;
				unset($state_info->search);
				$this->where[] = array(" sub_id in (6731)",null,null);
				$this->set_ajax_list_queries($state_info);
				//echo "<pre>";print_r($state_info);die();
	...
	//ruki
	public function shown_field_info($field_name, $shown_field_info = null)
	{
		if(is_array($field_name))
		{
			foreach($field_name as $field => $shown_field_info)
			{
				$this->shown_field_info[$field] = $shown_field_info;
			}
		}
		elseif($shown_field_info !== null)
		{
			$this->shown_field_info[$field_name] = $shown_field_info;
		}
		return $this;
	}
