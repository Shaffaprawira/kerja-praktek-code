						...
						<table  cellpadding="0" cellspacing="0">
							...
							<tr>
								<td class="content-block">
									<h5 style="color: #333; font-style: italic; padding: 10px; margin-bottom: 10px; border-bottom: solid 1px #DDD; background-color: #F7F7F7; font-weight: normal;"><i class="fa fa-file-text-o"></i> <?=$s?></h5>
								</td>
							</tr>
								<tr>
<td class="content-block">
	<p>Dear <?=trim($u->salutation.' '.ucwords($u->name))?>,</p>
	<?php
		$listInSection = '';
		$listOutSection = '';
		$done = array();
		$issue = 'a';
		$tmp = explode(',',$u->section);
		if(empty($tmp)){
		    $tmp[] = $u->section;
		}
		foreach($tmp as $sec){ //utk tiap section yg orang ini pernah submit
			foreach($p as $ppr){ //utk tiap paper di issue ini
				if($ppr->section_id == $sec){
					$done[] = $ppr->sub_id;
					$listInSection .= "<p><a href='$ppr->doi_url' target='_blank' title='See publisher page'><b>$ppr->title</b></a><br><i>". $this->lib_view->author_submission($ppr->sub_id) ."</i></p>";
				}
			}
		}
		foreach($p as $ppr){ //utk tiap paper di issue ini
			if(!in_array($ppr->sub_id,$done)){ //yg tidak termasuk di section of interest dari orang ini
				$listOutSection .= "<p><a href='$ppr->doi_url' target='_blank' title='See publisher page'><b>$ppr->title</b></a><br><i>". $this->lib_view->author_submission($ppr->sub_id) ."</i></p>";
			}
		}
		$issue = "Vol. $i->volume No. $i->issue_number ($i->year)";
		//TODO: contributing reviewer, all-time most downloaded, all-time most cited, info next CFP
		

	?>
	
	<p>We would like to inform you that International Journal of Technology (IJTech) <a href='<?=base_url('issue/'.$i->issue_id)?>' target='_blank'><b><?=$issue?></b></a> is published. Here are some articles that you might be interested in</p>
	<br>
	
	<?php if($listInSection!=''){ ?>
	<div style="background:#eee;border-radius:9px;padding:15px;font-size:large;border:1px solid #aaa">
		<span style="font-size:larger">Article(s) within your area of expertise</span><br>
		<span style="font-size:10px;color:#888">(based on the section that you chose when submitting your manuscript)</span><br><br>
		<?=$listInSection?>
	</div>
	<?php } ?>
	<div style="padding:15px">
	    <?php if($listInSection!=''){ ?>
		<span style="font-size:large;margin-bottom:12px">Other articles:</span><br><br>
		<?php } ?>
		<?=$listOutSection?>
	</div>
	
	<p>We hope this information can be useful in advancing your research.</p>
									</td>
							</tr>                                
							<tr>
	...
	<p>
		...
		<a href="<?php echo site_url();?>" target="_blank"><?php echo site_url();?></a>
				...
				<div class="footer">
					<table width="100%">
						<tr>
							<td class="aligncenter content-block">IJTech is currently indexed in SCOPUS and Emerging Sources Citation Index (ESCI) Thomson Reuters. <a href="<?=base_url('dashboard')?>">Click here</a> if you do not want to receive any published issue informations from us.</td>
							<?php 
